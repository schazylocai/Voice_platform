...
def read_subscription_from_azure_keyvault_arabic(user_email, user_password):
    try:
...
def write_subscription_ids_to_azure_keyvault_arabic(user_email, user_password):
    try:
        # Clean the user_email to use it as the secret name
        username = re.sub(r'\W+', '', user_email)
        ...
        try:
            ...
            change_text_style_arabic_side("تم حفظ البيانات", 'text_red_side', red)
...
def retrieve_password_from_azure_keyvault_arabic(user_email):
    try:
        # Clean the user_email to use it as the secret name
        username = re.sub(r'\W+', '', user_email)
        # Check if the secret already exists in the Key Vault
        try:
            ...
            # If the secret doesn't exist, write it to the Key Vault
            return user_email, "كلمة المرور غير موجودة"
